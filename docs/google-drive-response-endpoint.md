...
- スプレッドシート「回答ログ」への追記（回答本文・楽曲情報・添付URLも記録）
...
├─ 回答ログ             スプレッドシート（受信履歴・回答詳細）
└─ 2026-07-23/          放送回ごとの画像などの添付ファイル
```

## 回答ログに記録される内容

`回答ログ` は受信確認用の見やすい一覧です。次の送信分から、既存の基本列に加えて右側へ詳細列が追加されます。

- 受付ID、フォーム名、フォーム種別、放送回、応募期間、フォーム受付期間
- 公開プロフィール、記事/SNS用回答、制作側メモ、触れないこと・表記ルール
- 楽曲情報、全質問回答、添付ファイルURL、JSON保存先URL

回答の正本は常に `_responses/` のJSONです。シート上で長すぎる回答は省略されることがありますが、JSONには全文が残ります。

同じスプレッドシート内に `回答詳細` タブも作られます。こちらは「1質問=1行」で、質問内容・用途・入力形式・回答・楽曲URL・大容量音源URL・添付URLを確認できます。
...
回答ログの列を更新したあと、過去のJSONからシートを作り直したい場合はApps Script画面で関数 `rebuildResponseLogFromJson` を選び、1回だけ「実行」してください。
`_responses/` に残っているJSONを読み直して、`回答ログ` と `回答詳細` を再作成します。
